...
test("keeps the completed platform and consumer journeys wired", async () => {
  ...
  assert.match(css, /@media \(max-width: 900px\) \{[\s\S]*?\.cycle-section-heading h2 \{[^}]*white-space: normal/);
  assert.match(css, /@media \(max-width: 900px\) \{[\s\S]*?\.role-cycle-stage \{[^}]*display: grid/);
  assert.match(css, /@media \(max-width: 900px\) \{[\s\S]*?\.role-cycle-node,[\s\S]*?width: 100%/);
  assert.match(css, /@media \(max-width: 900px\) \{[\s\S]*?\.role-cycle-detail-grid \{ grid-template-columns: 1fr; \}/);
  ...
  assert.doesNotMatch(demo, /previewMode|device-toggle|dashboard-device-preview|dashboard-device-toggle|切換功能頁裝置預覽|切換首頁裝置預覽/);
  assert.doesNotMatch(css, /device-preview-mobile|device-toggle|dashboard-device-preview|dashboard-device-toggle/);
